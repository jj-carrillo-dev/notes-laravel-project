            ...
            <div class="bg-white dark:bg-gray-800 p-6 overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('notebooks.store') }}" method="post">
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="name" name="name" class="w-full mt-1" placeholder="Title" value="{{ old('name') }}" />
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="order" name="order" type="number" class="w-full mt-1" placeholder="Order (Default: 9)" value="{{ old('order', 9) }}" />
                        @error('order')
                            <div class="text-sm mt-1 text-red-500">{{ $message }}</div>
                        @enderror
                    </div>

                    <x-primary-button class="mt-6">Create</x-primary-button>
                </form>
            </div>
        </div>
    </div>
